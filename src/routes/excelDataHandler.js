...
import { getNotAllowedStrings, getObjectModelsAndTypes } from '../config.js';
...
function readData() {
  ...
  logger.info(`Intend to import: ${path.join(__dirname, '../import/' + process.env.FILE)}`);
  let workbook = readFile(path.join(__dirname, '../import/' + process.env.FILE));
  let sheet_name_list = workbook.SheetNames;
  data.notAllowedStrings = getNotAllowedStrings();
  data.validObjectModels = getObjectModelsAndTypes();
  data.browserObjects = utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]], { defval: '', range: 2 });
...
async function createOPCfiles(fileName) {
  ...
  let wbClient06 = utils.book_new();
  let dataClient06 = utils.json_to_sheet(data.filteredObjects.filter(object => object.dataType === 'VT_BOOL' || object.dataType === 'VT_I4'));
  dataClient06['!autofilter'] = { ref: 'A1:I9' };
  utils.book_append_sheet(wbClient06, dataClient06, 'Client006_Management');
  writeFile(wbClient06, path.join(__dirname, '../download/client006.xlsx'));
  let wbAlias = utils.book_new();
  let aliasList = [];
  ...
  let alias = utils.json_to_sheet(aliasList);
  utils.book_append_sheet(wbAlias, alias, 'AliasList');
  writeFile(wbAlias, path.join(__dirname, '../download/alias_list.xlsx'));
...
function createClientExcel(data, clientNumber) {
  ...
  let data_R8 = utils.json_to_sheet(data.filter(object => object.dataType === 'VT_R8'));
  let data_UI4 = utils.json_to_sheet(data.filter(object => object.dataType === 'VT_UI4'));
  data_R8['!autofilter'] = { ref: 'A1:I9' };
  data_UI4['!autofilter'] = { ref: 'A1:I9' };
  utils.book_append_sheet(wb, data_R8, 'Client' + clientNumber + '_VT_R8');
  utils.book_append_sheet(wb, data_UI4, 'Client' + clientNumber + ' BAC_VT_UI4');
  writeFile(wb, path.join(__dirname, '../download/client0' + clientNumber + '.xlsx'));
}
